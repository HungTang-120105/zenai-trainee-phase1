...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        query_reshape = _query.reshape((N, S, self.n_head, self.head_dim)).permute(0,2,1,3)
        key_reshape = _key.reshape((N, T, self.n_head, self.head_dim)).permute(0,2,3,1)
        value_reshape = _value.reshape((N, T, self.n_head, self.head_dim)).permute(0,2,1,3)
        
        qk_T = torch.matmul(query_reshape, key_reshape)/ self.head_dim**0.5 # shape = (N, num_head, S, T)
        if attn_mask is not None:
          qk_T = qk_T.masked_fill(
                          attn_mask.unsqueeze(0).unsqueeze(0) == 0,
                          float('-inf')
                      ) # shape = (N, num_head, S, T)
        attn = torch.softmax(qk_T, dim = -1) 
        attn = self.attn_drop(attn)
        output = torch.matmul(attn, value_reshape) 
...
class PatchEmbedding(nn.Module):
    ...
    def forward(self, x):
        ...
        patches = patches.reshape(N, self.num_patches, patch_size * patch_size * C)
