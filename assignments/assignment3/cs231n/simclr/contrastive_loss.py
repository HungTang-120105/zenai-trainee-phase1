...
def sim(z_i, z_j):
    ...
    ##############################################################################
    norm_dot_product = z_i @ z_j / (torch.linalg.norm(z_i, dim=-1) * torch.linalg.norm(z_j, dim=-1))
...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        ##############################################################################
        sim_k_N = torch.exp(sim(z_k, z_k_N) / tau)
        ...
        loss_1 = -torch.log(sim_k_N / denominator_1)
        loss_2 = -torch.log(sim_k_N / denominator_2)
...
def compute_sim_matrix(out):
    ...
    numerator = out @ out.T
    denominator = torch.linalg.norm(out, dim = -1, keepdim= True) * torch.linalg.norm(out, dim = -1).squeeze(0)

    sim_matrix = numerator / denominator
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    spp = sim_positive_pairs(out_left, out_right).squeeze()
    # Step 3: Compute the numerator value for all augmented samples.
    numerator = None
    numerator = torch.exp(
      torch.cat([spp, spp], dim=0) / tau
    )
